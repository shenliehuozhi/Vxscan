...
def reverse_domain(host):
    ...
    try:
        ...
        r = requests.get('http://api.hackertarget.com/reverseiplookup/?q={}'.format(host), headers=HEADERS, timeout=5)
...
def virustotal(host):
    ...
    if resp.get('status_code') != 403:
        for i in resp.get('json_resp').get('resolutions'):
            ...
            result = True
            for cdn in cdns:
                if (ipaddress.ip_address(address) in ipaddress.ip_network(cdn)):
                    result = False
            if result:
